...
The input has an optional `name` field containing only meaningful artist, album, track, or playlist name text of at most 500 characters and 20 words, an optional single exact `genre`, an optional inclusive `fromYear`/`toYear` pair, and optional `rating`/`ratingMatch` fields that must be supplied together. At least a name, genre, or complete year pair is required; rating alone is not a starting point. Genre matching is trimmed and case-insensitive but otherwise exact. Year bounds accept full years from 1000 through next year or 0–99 shorthand resolved relative to the current UTC year; reversed effective bounds are repaired. `rating` is a decimal number from 0 to 5; `ratingMatch` is `exact` for exactly that rating or `at_least` for that rating and higher, so rating 4 with `at_least` means 4+. Ratings and other constraints belong in their separate fields, never in `name`; recognisable misplaced numeric rating syntax returns a corrective tool error. Genre, year, and rating constraints are conjunctive, apply to tracks, and suppress album and playlist results. Name-free genre/year searches draw varied results from the complete qualifying track population.
...
When the whole query uniquely and exactly identifies an artist, with no equally exact album or track, search draws from that artist's canonical complete track relationship instead of the bounded text-retrieval lanes and populates `exactArtistMatch`. For an unconstrained search, `albums` becomes a varied preview of up to five entries drawn through a bounded rotating pool from the complete canonical album-artist relationship. Guest-appearance-only albums are excluded. Its `discographyBrowseRef` opens the complete paged album-artist discography. Genre-, year-, or rating-constrained searches preserve the same exact-artist representation but return qualifying tracks only. Duplicate exact artists, fuzzy artist matches, and exact album or track conflicts—including self-titled albums—retain ordinary search behaviour.
