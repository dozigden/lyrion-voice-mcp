...
public sealed class SqliteCatalogueSearchIndex : ISearchResolver, IDiagnosticSearchResolver
{
        ...
        [EnumeratorCancellation] CancellationToken cancellationToken)
    {
        ...
        var constraintClause = kind switch
        {
            MediaEntityKind.Track => AddTrackConstraintClause(
                command,
                trackConstraint,
                "documents"),
            MediaEntityKind.Album => AddAlbumConstraintClause(
                command,
                albumConstraint,
                "documents"),
            _ => string.Empty
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            await foreach (var candidate in ReadAlbumsAsync(
                constraint.AlbumConstraint,
                cancellationToken))
            {
                matchedCount++;
                AddConstraintScanResult(
                    results,
                    candidate,
        ...
        await foreach (var candidate in ReadTracksAsync(
            constraint.TrackConstraint,
            cancellationToken))
        {
            matchedCount++;
            AddConstraintScanResult(results, candidate, LaneCandidateLimit);
        }
    ...
    private static void AddConstraintScanResult(
        ICollection<SearchDiagnosticCandidate> results,
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (offset < 0 || limit is < 1 or > 50)
        {
            throw new ArgumentOutOfRangeException(
                nameof(limit),
                "Rating browse requires a non-negative offset and a limit from 1 to 50.");
        }

        var minimum = bucket * 20;
        var maximum = bucket == 5 ? 100 : minimum + 19;
        await using var connection = new SqliteConnection(connectionString);
        await connection.OpenAsync(cancellationToken);
        await using var command = connection.CreateCommand();
        command.CommandText = """
            SELECT stable_key, title, artist, album, native_rating
            FROM documents
            WHERE kind = $trackKind
              AND native_rating BETWEEN $minimumRating AND $maximumRating
            ORDER BY
                native_rating DESC,
        ...
        command.Parameters.AddWithValue("$minimumRating", minimum);
        command.Parameters.AddWithValue("$maximumRating", maximum);
        command.Parameters.AddWithValue("$limit", limit + 1);
        command.Parameters.AddWithValue("$offset", offset);
        var items = new List<RatingBrowseTrack>(limit + 1);
        await using var reader = await command.ExecuteReaderAsync(cancellationToken);
        while (await reader.ReadAsync(cancellationToken))
        {
            var stableKey = reader.GetString(0);
            const string trackPrefix = "track:";
            if (!stableKey.StartsWith(trackPrefix, StringComparison.Ordinal)
                || stableKey.Length == trackPrefix.Length)
            {
                throw new InvalidDataException(
                    "A rating browse document has an invalid track identity.");
            }

            items.Add(new RatingBrowseTrack(
                new MediaIdentity(MediaEntityKind.Track, stableKey[trackPrefix.Length..]),
                reader.GetString(1),
                reader.IsDBNull(2) ? null : reader.GetString(2),
                reader.IsDBNull(3) ? null : reader.GetString(3),
                reader.GetInt32(4)));
        ...
        return new RatingBrowsePage(items, hasMore);
    }
        ...
        CancellationToken cancellationToken)
    {
        ...
        var nativeRatingFilter = NativeRatingFilter.Create(
            constraint?.TrackConstraint.RatingConstraint);
        ...
        CatalogueSearchConstraint? constraint)
    {
        ...
        var trackPredicate = AddTrackConstraintClause(
            command,
            constraint.TrackConstraint,
            "filtered_documents");
        var predicates = new List<string>
        {
            $"(filtered_documents.kind = $constraintTrackKind {trackPredicate})"
        };
        if (constraint.AlbumConstraint is not null)
        {
            command.Parameters.AddWithValue(
                "$constraintAlbumKind",
                (int)MediaEntityKind.Album);
            var albumPredicate = AddAlbumConstraintClause(
                command,
                constraint.AlbumConstraint,
                "filtered_documents");
            predicates.Add(
                $"(filtered_documents.kind = $constraintAlbumKind {albumPredicate})");
        }

        return $"""
            AND document_id IN (
                SELECT filtered_documents.document_id
                FROM documents AS filtered_documents
                WHERE {string.Join(" OR ", predicates)})
            """;
    }

    private static string AddAlbumConstraintClause(
        SqliteCommand command,
        CatalogueAlbumSearchConstraint? constraint,
        string documentAlias)
    {
        ...
        command.Parameters.AddWithValue("$constraintAlbumFromYear", constraint.FromYear);
        command.Parameters.AddWithValue("$constraintAlbumToYear", constraint.ToYear);
        return $"AND {documentAlias}.year BETWEEN "
            + "$constraintAlbumFromYear AND $constraintAlbumToYear";
    }
    ...
    private sealed record NativeRatingFilter(
        RatingMatchMode Match,
        int NativeRating,
        bool CanMatch)
    {
        public static NativeRatingFilter? Create(RatingSearchConstraint? constraint)
        {
            if (constraint is null)
            {
                return null;
            ...
            var scaled = constraint.Rating * 20m;
            if (constraint.Match == RatingMatchMode.Exact)
            {
                return decimal.Truncate(scaled) == scaled
                    ? new NativeRatingFilter(constraint.Match, (int)scaled, true)
                    : new NativeRatingFilter(constraint.Match, 0, false);
            }

            return new NativeRatingFilter(
                constraint.Match,
                (int)decimal.Ceiling(scaled),
                true);
        }
    }
}
