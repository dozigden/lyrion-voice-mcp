...
public sealed class McpEndpointTests : IClassFixture<LyrionVoiceMcpApiFactory>
{
    ...
    public async Task ListToolsShouldAdvertiseTheImplementedToolSurface()
    {
        ...
        Assert.Contains(searchOutput.GetProperty("required").EnumerateArray(), property => property.GetString() == "bbcSoundsStations");
        var stationSchema = searchOutput.GetProperty("properties").GetProperty("bbcSoundsStations").GetProperty("items");
        Assert.True(stationSchema.GetProperty("properties").TryGetProperty("browseRef", out _));
        Assert.True(stationSchema.GetProperty("properties").TryGetProperty("playRef", out _));
    ...
    public async Task SearchToolShouldReturnStructuredMinimalCandidates()
    {
        ...
        var station = Assert.Single(structuredContent.GetProperty("bbcSoundsStations").EnumerateArray());
        Assert.Equal("Fictional Radio", station.GetProperty("name").GetString());
        Assert.Equal("station-reference", station.GetProperty("browseRef").GetString());
        Assert.Equal("station-reference", station.GetProperty("playRef").GetString());
    ...
    private sealed class StubSearchService(SearchOutcome? outcome = null) : ISearchService
    {
            ...
            CancellationToken cancellationToken)
        {
            ...
            [
                ...
                new(
                    "station-reference",
                    MediaEntityKind.Station,
                    "Fictional Radio",
                    null,
                    null,
                    ProviderId: "bbc-sounds")
            ],
            [
                new(
                    "top-track-reference",
