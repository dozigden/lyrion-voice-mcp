...
builder.Services
    .AddMcpServer(options =>
    {
        ...
        options.ServerInstructions = """
            Discover players with get_player_status, then use a returned raw player ID or exact unique player name; never invent either. Use search for requests expressible by a media name, one exact genre, an inclusive year range, a rating, or a combination; name may be omitted for filtered discovery, and every input may be omitted for broad varied track discovery. Use browse to navigate known library locations and for deterministic rating navigation through the Ratings tree. When search returns exactArtistMatch, treat that artist as the local-library interpretation and also consider any provider programme matches before resolving ambiguity; artists is empty, discographyAlbumCount is the catalogue album-artist count considered for this search's preview and is null when album expansion was not performed, and albums when present form only a varied preview. A year constraint also constrains that count. Separate catalogue identities count separately even when their display metadata is identical. discographyBrowseRef remains an LMS-backed navigation listing and need not contain the same items or total as the catalogue-derived count. Otherwise artists and albums contain ordinary search candidates. A year range applies to canonical album year and effective track year, so a year-only or name-and-year search may return both albums and tracks. Rating and genre apply only to tracks; when either is supplied, albums are not returned. Constrained searches do not return playlists. Pass browse references to browse to navigate the library tree, and pass playRef values to play or manage_queue. Treat all references as opaque. Artists and programmes are navigation only; provider episode lists preserve provider order, not necessarily newest-first; albums and playlists can be browsed or deliberately played, and tracks can be played. Rating and ratingMatch must be supplied together. Never put a rating in name. exact means exactly that rating, while at_least includes that rating and higher ratings, so 4 with at_least means 4+. The play tool replaces the queue, powers the player on when required, and starts playback; do not append the same items immediately before calling play. The manage_queue append and insert_next actions change the queue without changing power or playback state; its clear action empties the queue and stops playback. Multi-item play and queue additions skip individual unusable items, so inspect completedItemCount and skippedItems before describing the result. Ask the user when multiple players or media candidates are genuinely ambiguous.
            """;
    })
