...
- Services and Lms may depend on Abstractions. Services may additionally depend on `LyrionVoiceMcp.Ef.Abstractions` for repository contracts and persistence entities, but not on the EF implementation.
...
- MCP media and browse references use a bounded singleton in-memory handle registry in Services. The registry deliberately remains process-local because the runtime is one application server and handle invalidation on restart is part of the contract; do not move these ephemeral hand-off values into operational persistence. Tool-call history may persist bounded display metadata captured from a live handle when a call starts, but that diagnostic snapshot never restores or replaces the handle.
- The EF application database is authoritative for catalogue, search-observation, and operational data. Legacy database paths are not configured or read; old files are left untouched for manual disposal.
...
- BBC-specific public search naming is confined to Contracts and its dedicated API mapper. Tool-log provider schema and labelled result mapping for subscriptions and stations live in a feature-owned provider module; shared renderers consume those groups.
- BBC subscription and station persistence is canonical EF data; each has its own immutable, separately published disposable search snapshot. Provider availability never gates local catalogue or index readiness. No runtime plugin loader or new projects are required.
