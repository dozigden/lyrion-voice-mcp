...
- The deployed `/api/evaluation` diagnostics intentionally build disposable comparator indexes from the deployment's canonical catalogue so measurements reflect the target hardware and real deployed library. They must still refuse a non-converged or changing catalogue refresh.
