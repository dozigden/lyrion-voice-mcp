...
- BBC Sounds owns `bbc-sounds.refresh` and `bbc-sounds.restore-index`. Refresh reads subscriptions and stations before replacing their independently staged snapshots, records unavailable/empty success separately from failed reads, and preserves the last successful provider state on an LMS failure. Restoration reads both canonical EF snapshots without contacting LMS. Existing job results and logs expose provider outcomes; there is no provider enable switch or separate schedule.
