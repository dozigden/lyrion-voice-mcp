...
- `catalogue-phuzzy` reuses the same candidate source and adds experimental transliteration, query-span, joined-token, trigram, consonant-skeleton, and uppercase spoken-alias signals with application-owned ranking. Keep these signals separately observable in code and do not describe the simple consonant skeleton as a general phonetic or language-aware solution.
