...
- Retain private-corpus coverage for representative Roman-numeral titles using literal Roman, Arabic, spoken-cardinal, artist-qualified, and embedded contextual-title query forms where the real catalogue supports them. When the real catalogue has no genuine embedded contextual title, record that evidence limitation privately rather than fabricating a case. Keep fictional Search tests authoritative for the 1–50 boundary, standalone `I` exclusion, retained `V`/`X`, signed/dotted exclusions, canonical stored form, complete-context rule, and ranking/evidence order; never copy the private titles into public fixtures or guidance.
