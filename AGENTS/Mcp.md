...
- Every track search result has a `rating` string. Positive native LMS ratings are divided by 20 and formatted on the 0–5 star scale without unnecessary trailing zeroes; missing or zero ratings are `unrated`. Omit `rating` from artist, album, and playlist results, and never expose the native 0–100 value through MCP.
