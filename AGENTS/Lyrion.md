...
- The separate evaluation executable reads only `LVM_EVALUATION_LMS_BASE_URL` and uses a fixed `live-evaluation` identity. It must not inherit normal application LMS configuration or fall back to the development LMS.
...
- `LmsCatalogueReader` builds the provisional storage-neutral catalogue import snapshot through sequential paged `artists`, `albums`, `genres`, `titles`, and virtual-library membership queries. The unpaged `libraries` command is read once. LMS's configurable `artists` result is used only as a name lookup; the snapshot retains entries referenced by track `artist_ids` or album `artist_id`. Composer, conductor, band, other contributor-role fields, and unreferenced lookup entries are deliberately ignored. The reader also retains all genre IDs, provider/file metadata, and native rating/play-count values. It checks server status before and after the read, refuses to import during a scan, and emits only aggregate sanitised referential warnings. Manual refresh through `POST /api/catalogue/refresh` persists the snapshot atomically; it is not invoked automatically.
