...
- Jobs, schedules, errors, and MCP-call history use the EF application database. Retention, JSON-bound, timezone, and schedule settings live below `LyrionVoiceMcpOperations`. Full catalogue refresh is enabled daily at 03:00 by default when LMS is configured and is effectively disabled otherwise; the lightweight `Schedules:CatalogueChangeCheck` is independently enabled every 15 minutes. Either may be disabled or assigned another cron expression through configuration. Independently, startup readiness prepares the catalogue automatically when LMS is configured and no successful catalogue exists.
