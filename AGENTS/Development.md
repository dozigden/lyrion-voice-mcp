...
- Jobs, schedules, errors, and MCP-call history use the EF application database. Retention, JSON-bound, timezone, and schedule settings live below `LyrionVoiceMcpOperations`; catalogue automatic scheduling is disabled by default. Enabling the catalogue schedule also permits startup readiness to queue a refresh when no successful catalogue exists.
