...
- Compose accepts optional `LVM_SEARCH_RETENTION_DAYS`; EF-backed operational search history defaults to 90 days.
- Jobs, schedules, errors, and MCP-call history use the EF application database. Retention, JSON-bound, timezone, and deployment-default schedule settings live below `LyrionVoiceMcpOperations`. Full catalogue refresh is enabled daily at 03:00 by default when LMS is configured and is effectively disabled otherwise; the lightweight `Schedules:CatalogueChangeCheck` is independently enabled every 5 minutes. Environment or appsettings cron values remain deployment defaults until an operator saves that catalogue schedule through administration, after which the durable enabled state and canonical cron take precedence. Independently, startup readiness prepares the catalogue automatically when LMS is configured and no successful catalogue exists.
