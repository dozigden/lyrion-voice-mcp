...
- Feature-owned `BbcStations` rows use the same bounded staging pattern. `BbcStationState` selects the active snapshot, availability and expected station count; each row retains the stable live identity, display name and exact plugin-issued live audio locator.
- Search observation candidates retain an optional `ProviderId`; null continues to identify local-library candidates. Programme, episode and station are generic media kinds.
