...
- Remote comparator evaluation uses the Evaluation project's diagnostic REST host. Its result set is the full bounded retrieval union, not the whole catalogue or the public MCP result limit. Keep per-lane retrieval evidence and shared-scorer evidence available there, without copying a private corpus into this repository.
