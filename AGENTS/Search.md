...
- Remote comparator evaluation uses the deployed API's diagnostic REST endpoints backed by its canonical catalogue. Its result set is the full bounded retrieval union, not the whole catalogue or the public MCP result limit. Keep per-lane retrieval evidence and shared-scorer evidence available there, without copying a private corpus into this repository.
