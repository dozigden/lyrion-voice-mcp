...
import { catalogueHeaderStatus, indexHeaderStatus } from '../features/operations/headlineStatus';
...
<template>
  <div class="app-shell" :class="{ 'log-shell': isLog, 'system-list-shell': isSystemList }">
    <Header><template #actions>
      ...
      <div class="health" aria-label="System status"><RouterLink to="/system"><span class="dot" :class="{ ready: catalogue.ready }" aria-hidden="true"></span>Catalogue <time v-if="catalogue.timestamp" :datetime="catalogue.timestamp" :title="formatDate(catalogue.timestamp)">{{ catalogue.label }}</time><span v-else>{{ catalogue.label }}</span></RouterLink><RouterLink to="/system"><span class="dot" :class="{ ready: index.ready }" aria-hidden="true"></span>Index <time v-if="index.timestamp" :datetime="index.timestamp" :title="formatDate(index.timestamp)">{{ index.label }}</time><span v-else>{{ index.label }}</span></RouterLink></div>
...
.health { margin-left:auto; display:flex; flex-wrap:wrap; gap:20px; font-size:14px; }.health a { display:flex; align-items:center; gap:7px; color:#f6f0e5; text-decoration:none; }.health a > span:last-child,.health a > time:last-child { color:#b9b1a3; }.dot { width:6px; height:6px; border-radius:50%; background:var(--amber); }.dot.ready { background:#88a28a; }
