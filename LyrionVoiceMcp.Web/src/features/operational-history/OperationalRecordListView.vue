<template>
  <main class="page">
    <header><p class="eyebrow">Operational history</p><h1>{{ heading }}</h1><p>{{ description }}</p></header>
    <aside v-if="retentionDays" class="notice">Records are retained locally for {{ retentionDays }} days.</aside>
...
const description = computed(() => props.kind === 'jobs'
  ? 'Inspect queued and completed work, payloads, results and progress logs.'
  : props.kind === 'errors' ? 'Inspect unexpected application failures and their correlations.'
    : 'Inspect every tool invocation, including complete arguments and returned results.');
